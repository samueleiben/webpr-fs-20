...
(() => {
  ...
  function Person(first, last) {
    this.firstname = first; // this refers to the function, which leads to the OOPS!
    this.lastname = last;
...
(() => {
  ...
  const good = {
    ...
    getName: function() {
      return this.firstname + ' ' + this.lastname; // this refers to the object that it was called on (whats on the left side of the dot)
    }
  };
...
(() => {
  ...
  function Person(first, last) {
    // safer, because firstname and lastname are only in the scope of the constructor function (and not in the returned one)
    let firstname = first;
    let lastname = last;
    return {
...
(() => {
  ...
  const Person = (() => {
    // lexical scope for construction
    function Person(first, last) {
      // constructor, setting up the binding
      this.firstname = first;
      this.lastname = last;
    }
    Person.prototype.getName = function() {
      // functions are shared through the prototype // "=>" not allowed!
      return this.firstname + ' ' + this.lastname;
    };
    return Person;
  })(); // IIFE
  ...
  ok.push(good instanceof Function === false); // why this ??? --> because good has been created with the "new" keyword, good refers to the object
